...
High (前提が正しい場合)。React `useState("")` 初期値、trim 漏れ、未入力プレースホルダの誤伝搬で容易に踏みうる。エンドユーザーからは「接続失敗」のみ観測でき原因特定に時間がかかる。ただし上記のとおり 2020.5.0 で意図的に空文字を許容した経緯があるため、Sora が現在空文字を拒否することを着手前に確認する (後述)。
...
`clientId` / `bundleId` に空文字を渡した場合は **Sora 側で接続を拒否する** ため、SDK 側で同期 `Error` を throw してまで早期検出する必要性は低い。Sora からの拒否レスポンスでも開発者は誤用に気付ける。加えて 2020.5.0 で `clientId` の空文字許容を意図的に導入した経緯があり (CHANGES.md 2020.5.0 `[UPDATE] clientId option に空文字列を渡せるように修正する`)、本 issue は後方互換のない `[CHANGE]` を伴うため安易には踏み込めない。SDK と Sora の二重バリデーション化と後方互換破棄のコストを天秤にかけ、現時点では Sora 側の拒否に委ねる方針とする。

着手判断のトリガー:

- Sora 側の拒否レスポンスだけでは開発者が誤用に気付けないという実例が出てきた場合
- 2020.5.0 で空文字許容を導入した背景 (どのユースケースを救済する目的だったか) を再確認し、現在もそのユースケースが有効か判断できた場合
- 後方互換のない `[CHANGE]` を出してでも SDK 側で同期 throw する利点が、保守コストを上回ると判断できた場合
