...
本 issue が前提とする「孤児化」は、コードレビューの結果 **実際には発生しない** と判明したため、ソースコードの変更は行わずクローズする。

### 検証で判明した実際の動作

`connect()` 内の `Promise.race([multiStream(...).finally(...), setConnectionTimeout(), monitorSignalingWebSocketEvent()])` (`src/publisher.ts:23-30`, `src/subscriber.ts:19-26`, `src/messaging.ts:22-29`) は配列要素を同期評価する。同期実行時には次が起きる:

1. `multiStream` の本体に入り `await this.disconnect()` で suspend (内部の `disconnect` IIFE も最初の `await this.disconnectWebSocket("NO-ERROR")` で suspend)
2. `setConnectionTimeout` が同期実行され `setTimeout` を予約し、ID を `connectionTimeoutTimerId` に保存
3. `monitorSignalingWebSocketEvent` が同期実行され `setInterval` を予約し、ID を `monitorSignalingWebSocketEventTimerId` に保存

同期実行終了後、マイクロタスクキューが処理される:

4. `disconnectWebSocket` は `!this.ws` ガード (`src/base.ts:880`) で同期 `resolve(null)`、`await` の continuation がマイクロタスクで再開
5. `disconnect` IIFE の続きが走り、その末尾で `this.initializeConnection()` が呼ばれる
6. `initializeConnection` 末尾の **既存** `this.clearConnectionTimeout()` (`src/base.ts:851`) が、たった今予約した `setTimeout` を解除する
7. `setInterval` / `setTimeout` のコールバックはマクロタスクで、マイクロタスク処理後にしか発火しないため、上記 clear によって **発火する機会なく死ぬ**

すなわち 1 回目 connect 時点で `setInterval` / `setTimeout` は `initializeConnection` の `clearConnectionTimeout()` 経由で発火前に消えている。本 issue 本文「`### 孤児化が成立する機序`」で想定する「interval が `if (!this.ws) return` で空回りし自己 clear しない」は技術的に成立しない。

その結果として「2 回目 connect で `setInterval` がフィールド上書き → 1 回目 ID が失われて孤児化」というシナリオも、1 回目 interval が生存していないため発生しない。

### 残る別問題 (本 issue スコープ外)

1 回目 connect の `setConnectionTimeout` / `monitorSignalingWebSocketEvent` の Promise はそれぞれ dangling のままとなる (timer は clear 済みなので reject / resolve しない)。`Promise.race` は `multiStream` の解決で完了するため致命的ではないが、本 issue 本文が前提とする「`setConnectionTimeout` のタイムアウトで reject される 1 回目」も実際は dangling のため reject しない。

これは `disconnect` を `multiStream` 1 行目に挟みつつ、同一 race 内で `setConnectionTimeout` / `monitorSignalingWebSocketEvent` を起動する race 構造そのものに起因する構造的問題で、本 issue で提案していた冒頭 clear や `initializeConnection` への補助 clear 追加では解消しない。issue 0030 の冪等化リファクタ、あるいは別 issue で取り扱うべき領域。

### 変更ファイル

- `issues/0011-bug-fix-monitor-signaling-ws-event-interval-orphan.md` → `issues/closed/` へ移動
- ソースコード変更なし
- `CHANGES.md` への追記なし
