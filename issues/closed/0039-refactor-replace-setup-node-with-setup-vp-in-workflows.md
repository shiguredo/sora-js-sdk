...
- `with:` パラメータは現状の値を保ったまま渡す。具体的には:
  ...
  - `registry-url: https://registry.npmjs.org` (`npm-publish-canary` / `npm-publish` ジョブ): `voidzero-dev/setup-vp` の `registry-url` 入力に同じ値を渡す。`actions/setup-node` と同じく `$RUNNER_TEMP/.npmrc` に `registry=` を書き込む挙動を「## 着手前確認」で確認する。
...
1. `voidzero-dev/setup-vp@v1.10.0` の `action.yml` / README で以下の入力サポートを確認する。
   ...
   - `registry-url`: `https://registry.npmjs.org` を渡したときに `$RUNNER_TEMP/.npmrc` に `registry=https://registry.npmjs.org` を書き込むか (本リポジトリは OIDC publish で `NODE_AUTH_TOKEN` を使わないため `$RUNNER_TEMP/.npmrc` の registry 設定のみで足りる)。
...
- 全 11 ジョブで `actions/setup-node@48b55a011bda9f5d6aeb4c2d9c7362e8dae4041e # v6.4.0` を `voidzero-dev/setup-vp@ca1c46663915d6c1042ae23bd39ab85718bfb0fa # v1.10.0` に置換した
- 全 8 件の `pnpm/action-setup@0e279bb959325dab635dd2c09392533439d90093 # v6.0.8` の `uses` 行を削除した
- `pnpm install` / `pnpm add` / `pnpm remove` / `pnpm run *` / `pnpm exec *` を「コマンド対応表」に従って `vp` 経由に書き換えた
- 変更対象ファイル: `.github/workflows/ci.yaml` (ci / typescript-native-preview)、`e2e-test.yml` (e2e-test)、`e2e-test-canary.yml` (e2e-test-canary)、`e2e-test-h265.yml` (e2e-test-h265)、`e2e-test-webkit.yml` (e2e-test-webkit)、`npm-publish.yml` (verify-version / build / npm-publish-canary / npm-publish)、`npm-pkg-e2e-test.yml` (npm-pkg-e2e-test)
- 完了条件の grep カウント結果: `actions/setup-node` 0 件、`voidzero-dev/setup-vp` 11 件、`pnpm/action-setup` 0 件、`pnpm (install|run|exec|add|remove)` 0 件、`pnpm` 全般は `# pnpm publish は CI では正常に動作しない` 系のコメント 4 行のみ
- 着手前確認 1: `voidzero-dev/setup-vp@v1.10.0` の `action.yml` を確認した結果、`node-version` は 3 形態 (matrix 式 / 引用符付き文字列リテラル / 引用符なし数値リテラル) いずれも受け付けることを確認した。`registry-url` は `actions/setup-node` と同じく `$RUNNER_TEMP/.npmrc` に `registry=` を書き込む方式 (`~/.npmrc` ではない) のため、完了条件側の表現を `$RUNNER_TEMP/.npmrc` に統一し、canary tag push 時に `cat $RUNNER_TEMP/.npmrc` で書き込み内容を確認する
- 着手前確認 3: `sora-devtools` 側の 4 ワークフロー全てで `voidzero-dev/setup-vp@ca1c46663915d6c1042ae23bd39ab85718bfb0fa # v1.10.0` が使われ続けていることを確認した
- 完了条件のうち self-hosted runner (`e2e-test-h265.yml` / `e2e-test-webkit.yml`) の `workflow_dispatch` 起動、および `npm-publish.yml` の 4 ジョブ動作確認 (tag push でしか走らない) は本 PR の自動 CI では検証できないため、次回 canary tag push でのフォローアップ確認とする
- ステップ 2 のとおり `CHANGES.md` の `### misc` `[CHANGE]` 群末尾に追加エントリを追記した
- レビュー反映: 全 11 箇所の `voidzero-dev/setup-vp` の `with:` に `run-install: false` を明示し、setup-vp のデフォルト挙動 (`run-install: true`) による暗黙 `vp install` と手動 `vp install --frozen-lockfile` の二重実行を解消した
- レビュー反映: `e2e-test-h265.yml` / `e2e-test-webkit.yml` の `on:` に `workflow_dispatch:` を追加し、self-hosted runner ジョブを手動で起動できるようにした (完了条件「self-hosted runner の workflow_dispatch 起動」項目への対応)
- レビュー反映: `e2e-test-h265.yml` 冒頭のジョブコメントを実情に合わせて H265 を再生できる Chrome 各バージョン (Stable / Beta / Dev / Canary) を self-hosted Apple Silicon で検証する旨に書き換えた
- レビュー反映: `e2e-test-h265.yml` の `vp exec playwright test` 引数を `e2e-test-webkit.yml` と同じ単一行書式に統一した
- レビュー反映: 「### action の置き換え」「## 着手前確認」内の `~/.npmrc` 旧表記を `$RUNNER_TEMP/.npmrc` に統一した
...
- `npm-publish-canary` / `npm-publish` ジョブの `registry-url` 書き込み確認は `voidzero-dev/setup-vp` が `actions/setup-node` と同じく `$RUNNER_TEMP/.npmrc` に書き込む方式のため、本 PR の CI 上では検証できず、次回 canary tag push で `npm publish` 経路の動作確認とあわせてフォローアップする。
