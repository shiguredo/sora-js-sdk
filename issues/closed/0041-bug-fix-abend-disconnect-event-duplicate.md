...
`abend()` 内 abend 分岐終端 (`src/base.ts:858-860`) を、設計方針の diff 通り 1 行修正した。

- 修正前: `:860` で `this.callbacks.disconnect(this.soraCloseEvent("abend", title, params));` と `soraCloseEvent` を再呼び出ししており、`:858` の `event` (timeline に渡したインスタンス) と `:860` の `callbacks.disconnect` 引数が別インスタンスになっていた
- 修正後: `:860` を `this.callbacks.disconnect(event);` に書き換え、`:858` で生成した `event` を `:859` の `writeSoraTimelineLog` と `:860` の `callbacks.disconnect` で共有するよう変更した
- これにより abend 分岐の発火パターンが、normal 分岐 (`:853-855`)・`shutdown()` (`:744-748`)・`disconnect()` (`:1160-1164`) と完全に同じ「event 生成 → timeline → callback」(event 1 個共有) に揃った
- 値ベースの挙動 (`type` / `code` / `reason` / `title` / `params`) は変わらず、`callbacks.timeline` 経由で取得した `TimelineEvent.data` と `callbacks.disconnect` 引数の参照同一性 (`===`) が成立するようになった

### 検証

- `grep -n 'soraCloseEvent("abend"' src/base.ts` の出現箇所が修正前 4 件 (`:696`、`:858`、`:860`、`:1138`) から修正後 3 件 (`:696`、`:858`、`:1138`) に減ったことを確認した (`:860` の再呼び出しが消えた)
- `pnpm test` (vitest、3 ファイル / 105 件) が pass することを確認した
- `pnpm e2e-test` (playwright chromium、22 件 pass / 3 件 skip) が pass することを確認した
- `pnpm run lint` と `pnpm run typecheck` がエラーなしで通ることを確認した

### スコープ外として touch しなかった事項

- `abendPeerConnectionState` (`:696-698`) の callback / timeline 発火順逆問題: issue 0042 で対応
- 4 系統 (`abend` / `shutdown` / `disconnect` / `abendPeerConnectionState`) の `runShutdownOnce` への集約リファクタ: issue 0030 で対応
- `params["code"] === 1000` 分岐の dead code 整理: 0030 のスコープ
- normal 分岐 (`:848-856`) は元から event 1 個共有のため本修正では一切触らない

### テスト追加について

CLAUDE.md「モックやスタブは絶対に利用しないこと」規約により `callbacks` を呼び出し記録用の関数で差し替えるテストは採れず、abend を決定的に発火させる手段も限られるため新規テストは追加していない (issue「## テスト方針」に明示)。代わりに上記の `grep` カウント (4 件 → 3 件) と既存 `pnpm test` / `pnpm e2e-test` の pass で回帰を検証した。

### CHANGES.md の更新

`## develop` 配下、`[FIX]` 群末尾 (`trace() が JWT 等の機密を含む metadata を console / callbacks.log に raw 出力していたセキュリティ問題を redact で修正する` エントリ) の直後、`### misc` 見出しの直前に以下を追記した。
