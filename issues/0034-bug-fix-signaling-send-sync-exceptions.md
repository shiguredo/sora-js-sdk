...
```mermaid
flowchart TD
    subgraph funcs [対象 4 関数]
        F1[disconnectDataChannel]
        F2[sendSignalingMessage]
        F3[sendStatsMessage]
        F4[sendMessage public]
    end

    funcs --> A{compress / send}
    A -->|try/catch なし 現行| B[throw / unhandled rejection]
