...
`signalingTerminate()` (`src/base.ts:586-607`) は `this.ws.close()` を呼ぶだけで、`ws.onmessage` / `ws.onerror` を解除しない。一方、同じく接続を終了させる `abendPeerConnectionState()` (`src/base.ts:614-`) と `shutdown()` (`src/base.ts:678-`) は明示的に `ws.onmessage = null` / `ws.onerror = null` し、`ws.onclose` のみログ吐く専用クロージャに置き換えている。`signalingTerminate()` だけが非対称な実装になっており、broken window 状態である。
...
また、issue 0008 で導入された `src/base.ts:1284` のコメント `// signaling() の Promise が resolve / reject 済みかを追跡するフラグ。` は厳密には誤りで、`ws.onclose` 経由の reject や `createSignalingMessage` 失敗時の素 reject では `settled` を更新しない。実態は「`ws.onmessage` 内例外の pre-offer / post-offer 判別フラグ」が正確なので、同時に修正する。

## 優先度根拠

Medium。`connect()` のハングや内部状態破壊は引き起こさないため致命的ではない。ただし以下の理由で放置すべきではない:

- `shutdown` / `abendPeerConnectionState` と挙動が非対称で broken window 状態
...
### `signalingTerminate()` の実装 (`src/base.ts:586-607`)
...
### 既存の対称的な実装 (`abendPeerConnectionState` の場合、`src/base.ts:614-633` 抜粋)
...
`shutdown()` (`src/base.ts:738-746`) も同じパターン。`disconnect` 系経路 (`src/base.ts:2148-2150`, `:2171-2173`) は `ws.onclose / onmessage / onerror` をすべて null 化している。

### pre-offer reject 後の問題経路

issue 0008 マージ後の `signaling()` 内 (`src/base.ts:1277-1376`) で:
...
- L1323: OFFER 受信時、`resolve(message)` の直前
- L1350: REDIRECT 経路、`resolve(redirectMessage)` の直前
- L1294: `settleReject` 内 (pre-offer reject 時)

`settled` を更新しない reject 経路:

- `ws.onclose` (`src/base.ts:1301-1310`): `reject(error)` するが `settled` を更新しない
- `createSignalingMessage` 失敗 (`src/base.ts:1387-1390`): 素 `reject` で `settled` を更新しない
...
`shutdown` / `abendPeerConnectionState` と同じパターンを採用する。`ws.onclose` は「ログ吐く専用」クロージャに置き換える案と、現状維持の案がある。

#### 案 A: `shutdown` / `abendPeerConnectionState` と完全に同じパターン
...
メリット: `shutdown` / `abendPeerConnectionState` と完全に対称になり、broken window が解消される。
...
懸念: `shutdown` / `abendPeerConnectionState` と挙動が微妙に非対称 (broken window が完全に解消されない)。ただし「signaling() の Promise を確実に reject する」観点では現状の `ws.onclose` 経路を残す方が安全とも言える。

**推奨は案 A**。理由は (1) 既存の `shutdown` / `abendPeerConnectionState` と完全に対称にすることで broken window が解消する、(2) pre-offer reject 経路では `settleReject` が `signalingTerminate()` より先に `reject` 済みのため、`ws.onclose` を上書きしても settle 結果に影響しない。

### 修正 2: コメント `src/base.ts:1284` の精度向上
...
| ファイル      | 内容                                                                                    |
| ------------- | --------------------------------------------------------------------------------------- |
| `src/base.ts` | `signalingTerminate()` で `ws.onmessage` / `ws.onerror` を解除。コメント L1284 を修正。 |
...
- [ ] `signalingTerminate()` (`src/base.ts:586-607`) で案 A を採用し、`ws.onmessage = null; ws.onerror = null;` を追加し、`ws.onclose` を `writeWebSocketTimelineLog("onclose", ...)` だけのクロージャに置き換える
- [ ] コメント `src/base.ts:1284` を「ws.onmessage 内例外を pre-offer / post-offer のどちらとして扱うかを判別するフラグ」に修正する
- [ ] `shutdown` / `abendPeerConnectionState` の既存実装と整合する形にする (handler 解除パターン)

### 検証

- [ ] ローカルで `pnpm test` および `pnpm e2e-test` が通る (本修正は正常系の経路を変えないため非回帰確認のみ)
...
- `disconnect` 系経路 (`src/base.ts:2148-2150`, `:2171-2173`) の handler 解除パターン: 既に対称的に処理されており、変更不要
- `signalingTerminate()` 内の DataChannel 解除 (`src/base.ts:587-593`): 本 issue の対象外
