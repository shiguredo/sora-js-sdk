...
Medium。挙動修正ではなく観測装置の追加で実装は小さい。本 issue の観測値が関連 issue 0015 (mid 限定化を実装するか / workaround 自体を撤去するか / 現状維持か) の再開判断の入力となる (0015 は `issues/pending/` に退避済み。詳細は「関連 issue」参照)。加えて、将来 Sora 側仕様の変化により書き換えが暴発する事象が起きた際に timeline から即座に検知できる常時監視装置として機能する。

## 現状

### 該当箇所

`src/base.ts` の `processOfferSdp`: `isFirefox()` が真のとき、正規表現 `^m=(?<kind>audio|video) 0 /gmu` で port=0 の audio/video m 行を一括して port=9 に置換する。書き換えが発生したかどうかをランタイム側に伝える手段はない。

本メソッドは `setRemoteDescription` からのみ呼ばれ、`setRemoteDescription` は以下の経路で呼ばれる:

- 初回 offer 経路: `signalingOnMessageTypeOffer` は SDP に触れず `setRemoteDescription` も呼ばない。実際の `setRemoteDescription` 呼び出しは、resolve された offer message を受け取った各サブクラスの connect 経路 (`ConnectionPublisher` / `ConnectionSubscriber` / `ConnectionMessaging` それぞれの `multiStream` 内) で行われる
- re-offer 経路: `signalingOnMessageTypeReOffer`
- update 経路: `signalingOnMessageTypeUpdate` (`@deprecated`)
- simulcast 経路: `simulcast` 有効かつ role が `sendrecv` / `sendonly` のとき、`createAnswer` 内で transceiver 検索結果が非 `undefined` の場合のみ、再度 `setRemoteDescription(message)` を呼ぶ。**この経路により同一 `message.sdp` に対し `processOfferSdp` が最大 2 回呼ばれる**

### timeline log の既存仕組み

`writePeerConnectionTimelineLog(eventType, data)` を呼ぶと、`peerconnection` カテゴリの timeline event として `this.callbacks.timeline(event)` 経由で配信される。他の `peerconnection` カテゴリ event と同一経路。`data` は `createTimelineEvent` の中で `structuredClone` でコピーされる (循環参照等で失敗した場合は元参照を fallback)。

## 設計方針

### 純粋関数として `src/utils.ts` に切り出す

`processOfferSdp` は `private` メソッドかつテスト環境 (jsdom) では `RTCPeerConnection` を持たないため直接ユニットテストが書けない。AGENTS.md の「モックやスタブは絶対に利用しないこと」と整合させるため、書き換え判定 + 書き換え後 SDP + 集計結果を返す純粋関数を `src/utils.ts` に切り出す。関数自体はブラウザ非依存にし、Firefox 限定の発動制御は呼び出し側 (`processOfferSdp` orchestrator) の責務とする。
...
本関数も `rewriteOfferSdpPortZero` と同じく **`userAgent` を引数で受け取る純粋関数** として切り出し、テスト時に `navigator.userAgent` を差し替えなくて済むようにする (AGENTS.md のモック禁止規約と整合させるため)。呼び出し側 (`processOfferSdp` orchestrator) で `window.navigator.userAgent` を渡す。本ユーティリティは SDK 内部の workaround 観測用であり、利用者向け公開 API として露出させる用途は本 issue のスコープ外。
...
export function getFirefoxMajorVersion(userAgent: string): number | null {
  ...
  // noUncheckedIndexedAccess により match[1] は string | undefined になるため、
  // 型を絞ったうえでパースする (非 null アサーションを使わない規約に合わせる)
...
- `data` の TypeScript 型は専用 type を切らず、`writePeerConnectionTimelineLog` 呼び出し位置で inline object として渡す (既存 event も同様に inline object を渡しているものがある。例: `oniceconnectionstatechange` / `signaling-connection-timeout`。`set-remote-description` は `RTCSessionDescription` 変数を渡している点だけは異なる)
...
- recvonly は role 条件 (sendrecv / sendonly) で simulcast ブロックに入らず、messaging は role が `sendonly` 固定だが `setRemoteDescription` 後の `createAnswer` で transceiver 検索がマッチしない (メディアトラックを `addTrack` しない) ため、どちらも多重発火しない
...
- Firefox 以外のブラウザ向け workaround の観測 (例: answer SDP に stereo=1 を付与する `createAnswer` 内の `addStereoToFmtp` 呼び出し)。`options.forceStereoOutput` で制御され Chrome/Edge を想定した workaround だがブラウザ判定では発動しない点で本 issue の Firefox 限定 workaround とは性質が異なる。必要が生じたら別 issue で扱う
...
- `src/utils.ts` に純粋関数 `getFirefoxMajorVersion(userAgent: string): number | null` を追加する。配置位置は `isFirefox()` / `isChrome()` / `isSafari()` のブラウザ判定系 (`isFirefox` 関数定義) の直後、`createSignalingMessage` の前に揃える
...
- CHANGES.md の `## develop` セクション本体 (`### misc` ではない) の先頭 (現状は `[FIX]` エントリ 1 件のみで、その `[FIX]` エントリの前。`shiguredo-changelog` 規約の種別順 CHANGE → ADD → UPDATE → FIX に従った位置) に以下を追記する。`### misc` ではなく本体に置く理由: 本変更は `callbacks.timeline` に新規 eventType を発行する SDK の公開挙動の追加であり、SDK 利用者の観測可能な機能追加に該当するため (`shiguredo-changelog` 規約では機能に直接影響しない変更のみ `### misc` に置く)。担当者は実装者の GitHub ID に置き換えること:
...
0044 は 0015 と独立にマージ可能 (0015 は 2026-06-11 に `issues/pending/` へ退避済みのため、0044 が 0015 のブロッカーになる経路はない)。0044 で本番観測値を得てから 0015 の再開判断 (mid 限定化を実装するか / workaround 自体を撤去するか / 現状維持か) の入力とするのが望ましい。0015 が pending のままでも、0044 の価値は「workaround 撤去判断のための長期観測」として残る。

## 関連 issue

- **0015 (`feature/fix-process-offer-sdp-port-zero`、pending)**: 同じ `processOfferSdp` の port=0 書き換えを mid 限定化して RFC 3264 rejected 表明を保護するバグ修正。`issues/pending/` にあり、pending 理由は「現行 Sora は port=0 を transceiver 解放にしか使わず、一括置換が実害を起こす経路は本番に存在しないため」。着手判断のトリガーは「Sora が rejected 用途で port=0 を送る仕様を導入した / Firefox 113+ で port=0 のまま `onremovetrack` 不発が再現した / サポート下限 113+ で workaround の必要性を再検証して必要と判定された」のいずれかであり、0044 の観測値はその判断材料となる。0044 は 0015 を支援する観測装置の追加として独立した issue (1 issue 1 カテゴリ規約) で分離する
- **0045 (`feature/add-firefox-port-zero-rewrite-tests`、pending)**: 0044 で追加する `rewriteOfferSdpPortZero` / `getFirefoxMajorVersion` の単体テスト (vitest) と、`firefox-port-zero-rewrite` event 発火の Playwright Firefox による E2E テストを追加する。Playwright Firefox runner 整備 (`playwright.config.ts` のコメントアウト解除、CI workflow のマトリクス拡張) も本 issue ではなく 0045 で扱う。0044 マージ後の本番観測値が 0015 の再開判断にどの程度有用かを見極めてから、0045 の整備優先度を再検討する
